...
The Upgrade: Miden already relies on local, client-side proof generation. Ligero’s technology supercharges this, aiming to cut the time it takes to generate a proof on a phone or browser from 10-15 seconds down to around 1 second.
The article announces a strategic partnership between Miden and Ligero (a high-performance cryptography company whose tech is already used in Google Wallet). The goal of the collaboration is to build an institutional-grade "privacy stack" that makes real-world, compliant financial applications possible on-chain. By integrating Ligero’s advanced proving system into Miden’s architecture, proof generation on mobile devices and browsers becomes drastically faster and more secure.
